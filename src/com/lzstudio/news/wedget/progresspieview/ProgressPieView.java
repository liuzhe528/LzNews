package com.lzstudio.news.wedget.progresspieview;
...
public class ProgressPieView extends View {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mProgress = a.getInteger(R.styleable.ProgressPieView_ppvProgress,
				mProgress);
		mStartAngle = a.getInt(R.styleable.ProgressPieView_ppvStartAngle,
				mStartAngle);
		mInverted = a.getBoolean(R.styleable.ProgressPieView_ppvInverted,
				mInverted);
		mCounterclockwise = a.getBoolean(
				R.styleable.ProgressPieView_ppvCounterclockwise,
				mCounterclockwise);
		mStrokeWidth = a.getDimension(
				R.styleable.ProgressPieView_ppvStrokeWidth, mStrokeWidth);
		mTypeface = a.getString(R.styleable.ProgressPieView_ppvTypeface);
		mTextSize = a.getDimension(
				R.styleable.ProgressPieView_android_textSize, mTextSize);
		mText = a.getString(R.styleable.ProgressPieView_android_text);

		mShowStroke = a.getBoolean(R.styleable.ProgressPieView_ppvShowStroke,
				mShowStroke);
		mShowText = a.getBoolean(R.styleable.ProgressPieView_ppvShowText,
				mShowText);
	...
	protected void onDraw(Canvas canvas) {
		...
		mInnerRectF.offset((getWidth() - mViewSize) / 2,
		...
		case FILL_TYPE_RADIAL:
			...
			canvas.drawArc(mInnerRectF, mStartAngle, sweepAngle, true,
					mProgressPaint);
			break;
		case FILL_TYPE_CENTER:
			...
			break;
		default:
			throw new IllegalArgumentException("Invalid Progress Fill = "
					+ mProgressFillType);
		}
		...
		if (!TextUtils.isEmpty(mText) && mShowText) {
			...
			int yPos = (int) (centerY - (mTextPaint.descent() + mTextPaint
					.ascent()) / 2);
		...
		if (null != mImage && mShowImage) {
			...
			mImageRect.offset((getWidth() - drawableSize) / 2,
					(getHeight() - drawableSize) / 2);
	...
	public void animateProgressFill(int animateTo) {
		...
		if (animateTo > mMax || animateTo < 0) {
			throw new IllegalArgumentException(
					String.format(
							"Animation progress (%d) is greater than the max progress (%d) or lower than 0 ",
							animateTo, mMax));
		}
	 ...
	 * 
	 * @param inverted
	 *            draw the progress inverted or not
	 */
	 ...
	 *            draw the progress counterclockwise or not
	 */
